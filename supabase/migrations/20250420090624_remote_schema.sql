...
ALTER TYPE "public"."asphalt_type" OWNER TO "postgres";


COMMENT ON TYPE "public"."asphalt_type" IS 'Type of asphalt used';
...
ALTER TYPE "public"."existing_surface" OWNER TO "postgres";


COMMENT ON TYPE "public"."existing_surface" IS 'Type of surface present ';
...
COMMENT ON TYPE "public"."organization_role" IS 'Role in the contract (prime, sub..)';
...
ALTER TYPE "public"."patch_status" OWNER TO "postgres";
...
ALTER TYPE "public"."road_side" OWNER TO "postgres";
...
ALTER TYPE "public"."unit_measure_type" OWNER TO "postgres";


COMMENT ON TYPE "public"."unit_measure_type" IS 'Standardized units of measurement for construction quantities';
...
CREATE OR REPLACE FUNCTION "public"."calculate_cy"("length" numeric, "width" numeric, "depth" numeric) RETURNS numeric
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."calculate_cy"("length" numeric, "width" numeric, "depth" numeric) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."calculate_sy"("length" numeric, "width" numeric) RETURNS numeric
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."calculate_sy"("length" numeric, "width" numeric) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."calculate_tons"("volume" numeric, "density" numeric) RETURNS numeric
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."calculate_tons"("volume" numeric, "density" numeric) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."create_clone_for_test_user"("input_session_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
declare
  user_id constant uuid := '00000000-0000-0000-0000-000000000000';
  organization_id constant uuid := '14344b69-c36b-4e2a-880a-7b24effe1779';
  original_contract record;
  new_contract_id uuid := gen_random_uuid();
begin
  -- Verify profile exists and is an Admin
  if not exists (
    select 1
    from profiles
    where id = user_id
    ...
    raise exception 'Demo profile not found or not Admin';
  end if;

  -- Get the original demo contract
  select *
  into original_contract
  from contracts
  where title = 'Demo Contract Template'
  limit 1;

  if original_contract is null then
    raise exception 'Original demo contract not found';
  end if;

  -- Clone the contract using a new ID
  insert into contracts (
    ...
    new_contract_id,
    'Demo Contract - ' || input_session_id,
    original_contract.description,
    original_contract.location,
    original_contract.start_date,
    original_contract.end_date,
    user_id,
    now(),
    now(),
    original_contract.budget,
    original_contract.status,
    input_session_id
  );

  -- Link the contract to the demo org
...
ALTER FUNCTION "public"."create_clone_for_test_user"("input_session_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."create_clone_for_test_user"("session_id" "uuid", "user_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
declare
  original_contract contracts%rowtype;
  user_org_id uuid;
begin
  -- Get the original contract template
  select *
  into original_contract
  from contracts
  where title = 'Demo Contract Template';

  if original_contract.id is null then
    raise exception 'Original demo contract not found';
  end if;
  ...
  if user_org_id is null then
    raise exception 'No organization found for user %', user_id;
  end if;
...
ALTER FUNCTION "public"."create_clone_for_test_user"("session_id" "uuid", "user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."create_clone_for_test_user"("session_id" "uuid", "user_id" "uuid", "base_contract_id" "uuid", "base_organization_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
declare
...
begin
  -- Clone organization
  insert into organizations (id, name, address, phone, website)
  select new_organization_id, name, address, phone, website
  from organizations
  where id = base_organization_id;

  -- Clone contract
  insert into contracts (id, title, status, start_date, end_date, budget, organization_id, created_by)
  select new_contract_id, title || ' (CLONE)', status, start_date, end_date, budget, new_organization_id, user_id
  from contracts
  where id = base_contract_id;

  -- Clone contract_organizations
  insert into contract_organizations (contract_id, organization_id)
  values (new_contract_id, new_organization_id);

    -- Clone WBS entries and keep mapping for later
  create temporary table wbs_map (old_id uuid, new_id uuid);
  insert into wbs_map (old_id, new_id)
  select id, gen_random_uuid()
  from wbs
  where contract_id = base_contract_id;

  insert into wbs (id, contract_id, title, description)
  select wbs_map.new_id, new_contract_id, w.title, w.description
  from wbs w
  join wbs_map on w.id = wbs_map.old_id;

  -- Clone maps and map them to new WBS IDs
  create temporary table map_map (old_id uuid, new_id uuid);
  insert into map_map (old_id, new_id)
  select id, gen_random_uuid()
  from maps
  where wbs_id in (select old_id from wbs_map);

  insert into maps (
    id, wbs_id, contract_id, name, start_station, end_station, description
  )
  select 
    map_map.new_id,
    wbs_map.new_id,
    ...
    m.description
  from maps m
  join map_map on m.id = map_map.old_id
  join wbs_map on m.wbs_id = wbs_map.old_id;

  -- Clone line items under new maps
  insert into line_items (
    id, contract_id, wbs_id, map_id, title, description, unit, quantity, unit_price
  )
  select 
    gen_random_uuid(),
    new_contract_id,
    ...
    li.unit_price
  from line_items li
  join wbs_map on li.wbs_id = wbs_map.old_id
  join map_map on li.map_id = map_map.old_id;

  -- Clone base user profile details into new profile
  ...
  select
    ...
    full_name,
    email,
    'TEST.PROFILE',  -- Set username to constant
    phone,
    location,
    'Admin',
    job_title_id,
    ...
    true
  from profiles
  where id = '00000000-0000-0000-0000-000000000000';  -- Base demo user

  -- Assign new user to new contract
...
ALTER FUNCTION "public"."create_clone_for_test_user"("session_id" "uuid", "user_id" "uuid", "base_contract_id" "uuid", "base_organization_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."ensure_test_user_organization"() RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  user_id uuid;
...
ALTER FUNCTION "public"."ensure_test_user_organization"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."lock_budget_on_status_change"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."lock_budget_on_status_change"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."prevent_daily_log_if_inactive"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."prevent_daily_log_if_inactive"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."update_contract_budget"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."update_contract_budget"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."validate_formula_unit"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."validate_formula_unit"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."validate_formula_units"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."validate_formula_units"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."asphalt_types" (
...
ALTER TABLE "public"."asphalt_types" OWNER TO "postgres";


COMMENT ON COLUMN "public"."asphalt_types"."lift_depth_inches" IS 'Typical lift thickness for this mix type in inches';



COMMENT ON COLUMN "public"."asphalt_types"."target_spread_rate_lbs_per_sy" IS 'Recommended spread rate in lbs/SY based on average target depth';



COMMENT ON COLUMN "public"."asphalt_types"."jmf_temp_min" IS 'Minimum allowable JMF temperature for this mix';



COMMENT ON COLUMN "public"."asphalt_types"."jmf_temp_max" IS 'Maximum allowable JMF temperature for this mix';



COMMENT ON COLUMN "public"."asphalt_types"."compaction_min" IS 'Minimum compaction % of maximum specific gravity (Gmm)';
...
COMMENT ON COLUMN "public"."avatars"."is_preset" IS 'Indicates if this is a system-provided preset avatar (true) or user-uploaded (false)';



CREATE TABLE IF NOT EXISTS "public"."calculator_functions" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "name" "text" NOT NULL,
...
ALTER TABLE "public"."calculator_functions" OWNER TO "postgres";


COMMENT ON COLUMN "public"."calculator_functions"."id" IS 'Unique ID';



COMMENT ON COLUMN "public"."calculator_functions"."name" IS 'Name of the function (e.g., "Concrete Area Calculation").';



COMMENT ON COLUMN "public"."calculator_functions"."function_name" IS 'Name of the stored SQL function (e.g., calculate_sy).';



COMMENT ON COLUMN "public"."calculator_functions"."description" IS 'Explanation of what the function calculates.';



COMMENT ON COLUMN "public"."calculator_functions"."created_at" IS 'Timestamp when function was created.';
...
CREATE TABLE IF NOT EXISTS "public"."contract_organizations" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
...
ALTER TABLE "public"."contract_organizations" OWNER TO "postgres";


COMMENT ON TABLE "public"."contract_organizations" IS 'Junction table linking contracts to organizations';



COMMENT ON COLUMN "public"."contract_organizations"."id" IS 'Unique identifier for the contract-organization relationship';



COMMENT ON COLUMN "public"."contract_organizations"."contract_id" IS 'Reference to the contract';



COMMENT ON COLUMN "public"."contract_organizations"."organization_id" IS 'Reference to the organization';



COMMENT ON COLUMN "public"."contract_organizations"."notes" IS 'Additional notes about the organization''s involvement';



COMMENT ON COLUMN "public"."contract_organizations"."created_by" IS 'Reference to the user who created this relationship';



COMMENT ON COLUMN "public"."contract_organizations"."role" IS 'Role of Contractor on Contract';
...
COMMENT ON TABLE "public"."contracts" IS 'Construction contracts and their details';
...
COMMENT ON COLUMN "public"."contracts"."title" IS 'Contract title or number';



COMMENT ON COLUMN "public"."contracts"."description" IS 'Detailed description of the contract work';



COMMENT ON COLUMN "public"."contracts"."location" IS 'General project location';
...
COMMENT ON COLUMN "public"."contracts"."end_date" IS 'Contract end date';



COMMENT ON COLUMN "public"."contracts"."created_by" IS 'Reference to the user who created this contract';
...
COMMENT ON TABLE "public"."crew_members" IS 'Individual members assigned to crews';



COMMENT ON COLUMN "public"."crew_members"."id" IS 'Unique identifier for the crew member assignment';



COMMENT ON COLUMN "public"."crew_members"."crew_id" IS 'Reference to the crew';



COMMENT ON COLUMN "public"."crew_members"."profile_id" IS 'Reference to the crew member''s profile';



COMMENT ON COLUMN "public"."crew_members"."role" IS 'Member''s role within the crew';



COMMENT ON COLUMN "public"."crew_members"."assigned_at" IS 'Date and time of assignment';



COMMENT ON COLUMN "public"."crew_members"."created_by" IS 'Reference to the user who created this assignment';



COMMENT ON COLUMN "public"."crew_members"."map_location_id" IS 'Current work location assignment';



COMMENT ON COLUMN "public"."crew_members"."location_notes" IS 'Additional location details';
...
COMMENT ON TABLE "public"."crews" IS 'Work crews and their composition';
...
COMMENT ON COLUMN "public"."crews"."organization_id" IS 'Reference to the organization';



COMMENT ON COLUMN "public"."crews"."name" IS 'Crew name or number';



COMMENT ON COLUMN "public"."crews"."description" IS 'Description of crew specialty or purpose';



COMMENT ON COLUMN "public"."crews"."foreman_id" IS 'Reference to the crew foreman';



COMMENT ON COLUMN "public"."crews"."created_by" IS 'Reference to the user who created this crew';
...
CREATE TABLE IF NOT EXISTS "public"."dump_trucks" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "contract_id" "uuid",
    "equipment_id" "uuid",
...
ALTER TABLE "public"."dump_trucks" OWNER TO "postgres";


COMMENT ON COLUMN "public"."dump_trucks"."contract_id" IS 'Links the dump truck to a specific contract';



COMMENT ON COLUMN "public"."dump_trucks"."equipment_id" IS 'Optional FK to general equipment if applicable';



COMMENT ON COLUMN "public"."dump_trucks"."truck_identifier" IS 'Identifier such as number, make, or plate';



COMMENT ON COLUMN "public"."dump_trucks"."payload_capacity_tons" IS 'Payload capacity in tons';



COMMENT ON COLUMN "public"."dump_trucks"."notes" IS 'Any extra details about the dump truck';



COMMENT ON COLUMN "public"."dump_trucks"."bed_length" IS 'Length of dump truck bed (ft)';



COMMENT ON COLUMN "public"."dump_trucks"."bed_width" IS 'Width of dump truck bed (ft)';



COMMENT ON COLUMN "public"."dump_trucks"."bed_height" IS 'Height of dump truck bed (ft)';



COMMENT ON COLUMN "public"."dump_trucks"."bed_volume" IS 'Automatically calculated volume based on dimensions';



COMMENT ON COLUMN "public"."dump_trucks"."axle_count" IS 'Number of axles on the truck';



COMMENT ON COLUMN "public"."dump_trucks"."weight_capacity_tons" IS 'Maximum weight capacity in tons';



COMMENT ON COLUMN "public"."dump_trucks"."hoist_top" IS 'Width at the top of the dump bed used for volume calculation';



COMMENT ON COLUMN "public"."dump_trucks"."hoist_bottom" IS 'Width at the bottom of the dump bed used for volume calculation';



COMMENT ON COLUMN "public"."dump_trucks"."hoist_width" IS 'Length of the dump bed used in the hoist volume calculation';
...
COMMENT ON TABLE "public"."equipment_assignments" IS 'Tracks which equipment is assigned to which line item, with role, timing, and notes';
...
COMMENT ON TABLE "public"."job_titles" IS 'Predefined and custom job titles for users';



COMMENT ON COLUMN "public"."job_titles"."id" IS 'Unique identifier for the job title';



COMMENT ON COLUMN "public"."job_titles"."title" IS 'Name of the job title';



COMMENT ON COLUMN "public"."job_titles"."is_custom" IS 'Indicates if this is a custom job title created by a user';



COMMENT ON COLUMN "public"."job_titles"."created_by" IS 'Reference to the user who created this job title';



CREATE TABLE IF NOT EXISTS "public"."line_item_crew_assignments" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
...
ALTER TABLE ONLY "public"."line_item_crew_assignments" REPLICA IDENTITY FULL;


ALTER TABLE "public"."line_item_crew_assignments" OWNER TO "postgres";


COMMENT ON TABLE "public"."line_item_crew_assignments" IS 'Tracks which crews are assigned to which contract line items, including role, time window, and optional notes';



COMMENT ON COLUMN "public"."line_item_crew_assignments"."id" IS 'Unique identifier for the assignment';



COMMENT ON COLUMN "public"."line_item_crew_assignments"."line_item_id" IS 'Reference to the line item';



COMMENT ON COLUMN "public"."line_item_crew_assignments"."crew_id" IS 'Reference to the assigned crew';



COMMENT ON COLUMN "public"."line_item_crew_assignments"."start_date" IS 'Assignment start date';



COMMENT ON COLUMN "public"."line_item_crew_assignments"."end_date" IS 'Assignment end date';



COMMENT ON COLUMN "public"."line_item_crew_assignments"."status" IS 'Current status of the assignment';



COMMENT ON COLUMN "public"."line_item_crew_assignments"."notes" IS 'Additional notes about the assignment';



COMMENT ON COLUMN "public"."line_item_crew_assignments"."created_by" IS 'Reference to the user who created this assignment';



CREATE TABLE IF NOT EXISTS "public"."line_item_entries" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
...
ALTER TABLE "public"."line_item_entries" OWNER TO "postgres";


COMMENT ON COLUMN "public"."line_item_entries"."line_item_id" IS 'references the line item for this entry';



COMMENT ON COLUMN "public"."line_item_entries"."input_variables" IS 'List of the variables used in the calculator';



CREATE TABLE IF NOT EXISTS "public"."line_item_equipment_assignments" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "line_item_id" "uuid" NOT NULL,
    "equipment_id" "uuid" NOT NULL,
    "operator_id" "uuid",
...
ALTER TABLE ONLY "public"."line_item_equipment_assignments" REPLICA IDENTITY FULL;


ALTER TABLE "public"."line_item_equipment_assignments" OWNER TO "postgres";


COMMENT ON TABLE "public"."line_item_equipment_assignments" IS 'Mapping of equipment used on line items, allowing multiple pieces per task';



COMMENT ON COLUMN "public"."line_item_equipment_assignments"."id" IS 'Unique identifier for the assignment';



COMMENT ON COLUMN "public"."line_item_equipment_assignments"."line_item_id" IS 'Reference to the line item';



COMMENT ON COLUMN "public"."line_item_equipment_assignments"."equipment_id" IS 'Reference to the assigned equipment';



COMMENT ON COLUMN "public"."line_item_equipment_assignments"."operator_id" IS 'Reference to the equipment operator';



COMMENT ON COLUMN "public"."line_item_equipment_assignments"."start_date" IS 'Assignment start date';



COMMENT ON COLUMN "public"."line_item_equipment_assignments"."end_date" IS 'Assignment end date';



COMMENT ON COLUMN "public"."line_item_equipment_assignments"."status" IS 'Current status of the assignment';



COMMENT ON COLUMN "public"."line_item_equipment_assignments"."notes" IS 'Additional notes about the assignment';



COMMENT ON COLUMN "public"."line_item_equipment_assignments"."created_by" IS 'Reference to the user who created this assignment';
...
ALTER TABLE "public"."line_item_templates" OWNER TO "postgres";


COMMENT ON TABLE "public"."line_item_templates" IS 'Preset calculations for line items';



COMMENT ON COLUMN "public"."line_item_templates"."created_at" IS 'Timestamp when template was created.';



COMMENT ON COLUMN "public"."line_item_templates"."name" IS 'Name of the template (e.g., "Pavement Thickness Calculation").';



COMMENT ON COLUMN "public"."line_item_templates"."output_unit" IS 'Standardized output unit (SY, CY, TONS, etc.).';



COMMENT ON COLUMN "public"."line_item_templates"."calculator_function_id" IS 'References calculator_functions.id.';



COMMENT ON COLUMN "public"."line_item_templates"."description" IS 'Explanation of how this template is used.';



COMMENT ON COLUMN "public"."line_item_templates"."id" IS 'Unique ID for the line item templates';



COMMENT ON COLUMN "public"."line_item_templates"."unit_type" IS 'The unit of measurement for this line item (e.g., Lump Sum, SF, LF)';



COMMENT ON COLUMN "public"."line_item_templates"."formula" IS 'JSON structure describing expected inputs and formulas for automatic calculations';



COMMENT ON COLUMN "public"."line_item_templates"."instructions" IS 'Instructions for users filling out this template in the frontend';



COMMENT ON COLUMN "public"."line_item_templates"."organization_id" IS 'A way for an organization to have a list of templates they use all the time';



COMMENT ON COLUMN "public"."line_item_templates"."created_by" IS 'Who created this template';
...
COMMENT ON TABLE "public"."line_items" IS 'Individual work items within WBS sections';



COMMENT ON COLUMN "public"."line_items"."id" IS 'Unique identifier for the line item';



COMMENT ON COLUMN "public"."line_items"."wbs_id" IS 'Reference to the WBS section';



COMMENT ON COLUMN "public"."line_items"."line_code" IS 'Line item code or number';



COMMENT ON COLUMN "public"."line_items"."description" IS 'Description of the work item';



COMMENT ON COLUMN "public"."line_items"."unit_measure" IS 'Unit of measurement';



COMMENT ON COLUMN "public"."line_items"."quantity" IS 'Total quantity of work';



COMMENT ON COLUMN "public"."line_items"."unit_price" IS 'Price per unit';



COMMENT ON COLUMN "public"."line_items"."reference_doc" IS 'Reference to specifications or documents';



COMMENT ON COLUMN "public"."line_items"."template_id" IS 'Reference to line item template if used';



COMMENT ON COLUMN "public"."line_items"."map_id" IS 'Reference to the specific map location for this line item';
...
COMMENT ON TABLE "public"."maps" IS 'Physical locations and map references for work areas';



COMMENT ON COLUMN "public"."maps"."id" IS 'Unique identifier for the map location';



COMMENT ON COLUMN "public"."maps"."wbs_id" IS 'Reference to the WBS section';



COMMENT ON COLUMN "public"."maps"."map_number" IS 'Map or sheet number';



COMMENT ON COLUMN "public"."maps"."location_description" IS 'Description of the location';



COMMENT ON COLUMN "public"."maps"."coordinates" IS 'Geographic coordinates of the location';



COMMENT ON COLUMN "public"."maps"."budget" IS 'Null in Draft (dynamically calculated) Static After Draft Phase';
...
COMMENT ON TABLE "public"."organizations" IS 'Companies and organizations involved in projects';
...
COMMENT ON COLUMN "public"."organizations"."address" IS 'Organization''s physical address';



COMMENT ON COLUMN "public"."organizations"."phone" IS 'Organization''s contact phone number';



COMMENT ON COLUMN "public"."organizations"."website" IS 'Organization''s website URL';



COMMENT ON COLUMN "public"."organizations"."created_by" IS 'Reference to the user who created this organization';
...
CREATE TABLE IF NOT EXISTS "public"."profiles" (
    ...
    CONSTRAINT "check_valid_avatar_url" CHECK (("avatar_url" ~* '^(https?:\/\/).+'::"text"))
);
...
COMMENT ON TABLE "public"."profiles" IS 'Stores user profile information including role and contact details';



COMMENT ON COLUMN "public"."profiles"."id" IS 'Unique identifier for the profile, matches auth.users.id';



COMMENT ON COLUMN "public"."profiles"."role" IS 'User role determining permissions and access levels';
...
COMMENT ON COLUMN "public"."profiles"."location" IS 'User''s location or address';



COMMENT ON COLUMN "public"."profiles"."avatar_url" IS 'DEPRECATED: Use avatar_id instead. Will be removed in future version.';



COMMENT ON COLUMN "public"."profiles"."job_title_id" IS 'Reference to user''s job title';



COMMENT ON COLUMN "public"."profiles"."organization_id" IS 'Reference to user''s organization';



CREATE TABLE IF NOT EXISTS "public"."tack_rates" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
...
ALTER TABLE "public"."tack_rates" OWNER TO "postgres";


COMMENT ON COLUMN "public"."tack_rates"."surface_type" IS 'Type of surface the tack will be applied to (e.g., Milled Asphalt, Concrete)';



COMMENT ON COLUMN "public"."tack_rates"."application_rate" IS 'Tack application rate in gallons per square yard (GAL/SY)';



COMMENT ON COLUMN "public"."tack_rates"."notes" IS 'Optional notes regarding special conditions or exceptions';



COMMENT ON COLUMN "public"."tack_rates"."created_at" IS 'Timestamp when this tack rate was recorded';
...
COMMENT ON TABLE "public"."user_contracts" IS 'All user-contract relationships.';
...
COMMENT ON TABLE "public"."wbs" IS 'Work Breakdown Structure sections for contracts';



COMMENT ON COLUMN "public"."wbs"."id" IS 'Unique identifier for the WBS section';



COMMENT ON COLUMN "public"."wbs"."contract_id" IS 'Reference to the parent contract';



COMMENT ON COLUMN "public"."wbs"."wbs_number" IS 'WBS section number or identifier';



COMMENT ON COLUMN "public"."wbs"."scope" IS 'Description of the work in this WBS section';



COMMENT ON COLUMN "public"."wbs"."budget" IS 'Null in Draft (dynamically calculated) Static After Draft Phase';
...
ALTER TABLE ONLY "public"."calculator_functions"
    ADD CONSTRAINT "calculator_functions_function_name_key" UNIQUE ("function_name");



ALTER TABLE ONLY "public"."calculator_functions"
    ADD CONSTRAINT "calculator_functions_name_key" UNIQUE ("name");



ALTER TABLE ONLY "public"."calculator_functions"
    ADD CONSTRAINT "calculator_functions_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."line_item_crew_assignments"
    ADD CONSTRAINT "line_item_crew_assignments_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."line_item_equipment_assignments"
    ADD CONSTRAINT "line_item_equipment_assignments_pkey" PRIMARY KEY ("id");
...
CREATE OR REPLACE TRIGGER "handle_contract_organizations_updated_at" BEFORE UPDATE ON "public"."contract_organizations" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();



CREATE OR REPLACE TRIGGER "handle_contracts_updated_at" BEFORE UPDATE ON "public"."contracts" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();



CREATE OR REPLACE TRIGGER "handle_crew_members_updated_at" BEFORE UPDATE ON "public"."crew_members" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();



CREATE OR REPLACE TRIGGER "handle_crews_updated_at" BEFORE UPDATE ON "public"."crews" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();



CREATE OR REPLACE TRIGGER "handle_equipment_assignments_updated_at" BEFORE UPDATE ON "public"."equipment_assignments" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();



CREATE OR REPLACE TRIGGER "handle_line_item_crew_assignments_updated_at" BEFORE UPDATE ON "public"."line_item_crew_assignments" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();



CREATE OR REPLACE TRIGGER "handle_line_item_equipment_assignments_updated_at" BEFORE UPDATE ON "public"."line_item_equipment_assignments" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();



CREATE OR REPLACE TRIGGER "handle_line_items_updated_at" BEFORE UPDATE ON "public"."line_items" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();



CREATE OR REPLACE TRIGGER "handle_map_locations_updated_at" BEFORE UPDATE ON "public"."maps" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();
...
CREATE OR REPLACE TRIGGER "handle_wbs_updated_at" BEFORE UPDATE ON "public"."wbs" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();



CREATE OR REPLACE TRIGGER "trg_check_active_contract" BEFORE INSERT OR UPDATE ON "public"."daily_logs" FOR EACH ROW EXECUTE FUNCTION "public"."prevent_daily_log_if_inactive"();



CREATE OR REPLACE TRIGGER "update_contract_budget_delete" AFTER DELETE ON "public"."line_items" FOR EACH ROW EXECUTE FUNCTION "public"."update_contract_budget"();



CREATE OR REPLACE TRIGGER "update_contract_budget_insert" AFTER INSERT ON "public"."line_items" FOR EACH ROW EXECUTE FUNCTION "public"."update_contract_budget"();



CREATE OR REPLACE TRIGGER "update_contract_budget_trigger" AFTER INSERT OR DELETE OR UPDATE ON "public"."line_items" FOR EACH ROW EXECUTE FUNCTION "public"."update_contract_budget"();



CREATE OR REPLACE TRIGGER "update_contract_budget_update" AFTER UPDATE OF "quantity", "unit_price" ON "public"."line_items" FOR EACH ROW EXECUTE FUNCTION "public"."update_contract_budget"();



CREATE OR REPLACE TRIGGER "update_job_titles_updated_at" BEFORE UPDATE ON "public"."job_titles" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();
...
ALTER TABLE ONLY "public"."line_item_crew_assignments"
    ADD CONSTRAINT "line_item_crew_assignments_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."profiles"("id");



ALTER TABLE ONLY "public"."line_item_crew_assignments"
    ADD CONSTRAINT "line_item_crew_assignments_crew_id_fkey" FOREIGN KEY ("crew_id") REFERENCES "public"."crews"("id");



ALTER TABLE ONLY "public"."line_item_crew_assignments"
    ADD CONSTRAINT "line_item_crew_assignments_line_item_id_fkey" FOREIGN KEY ("line_item_id") REFERENCES "public"."line_items"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."line_item_equipment_assignments"
    ADD CONSTRAINT "line_item_equipment_assignments_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."profiles"("id");



ALTER TABLE ONLY "public"."line_item_equipment_assignments"
    ADD CONSTRAINT "line_item_equipment_assignments_line_item_id_fkey" FOREIGN KEY ("line_item_id") REFERENCES "public"."line_items"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."line_item_equipment_assignments"
    ADD CONSTRAINT "line_item_equipment_assignments_operator_id_fkey" FOREIGN KEY ("operator_id") REFERENCES "public"."profiles"("id");



ALTER TABLE ONLY "public"."line_item_templates"
    ADD CONSTRAINT "line_item_templates_calculator_function_id_fkey" FOREIGN KEY ("calculator_function_id") REFERENCES "public"."calculator_functions"("id");
...
CREATE POLICY "Line item crew assignments are viewable by organization members" ON "public"."line_item_crew_assignments" FOR SELECT TO "authenticated" USING (("line_item_id" IN ( SELECT "li"."id"
   FROM ((("public"."line_items" "li"
     JOIN "public"."wbs" "w" ON (("w"."id" = "li"."wbs_id")))
...
CREATE POLICY "Line item equipment assignments are viewable by organization me" ON "public"."line_item_equipment_assignments" FOR SELECT TO "authenticated" USING (("line_item_id" IN ( SELECT "li"."id"
   FROM ((("public"."line_items" "li"
     JOIN "public"."wbs" "w" ON (("w"."id" = "li"."wbs_id")))
     JOIN "public"."contracts" "c" ON (("c"."id" = "w"."contract_id")))
     JOIN "public"."contract_organizations" "co" ON (("co"."contract_id" = "c"."id")))
...
CREATE POLICY "Project managers can manage line item crew assignments" ON "public"."line_item_crew_assignments" TO "authenticated" USING (("public"."check_is_admin"() OR (EXISTS ( SELECT 1
   FROM (((("public"."line_items" "li"
     JOIN "public"."wbs" "w" ON (("w"."id" = "li"."wbs_id")))
     JOIN "public"."contracts" "c" ON (("c"."id" = "w"."contract_id")))
     JOIN "public"."contract_organizations" "co" ON (("co"."contract_id" = "c"."id")))
     JOIN "public"."profiles" "p" ON (("p"."organization_id" = "co"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = 'Project Manager'::"public"."user_role") AND ("li"."id" = "line_item_crew_assignments"."line_item_id")))))) WITH CHECK (("public"."check_is_admin"() OR (EXISTS ( SELECT 1
   FROM (((("public"."line_items" "li"
     JOIN "public"."wbs" "w" ON (("w"."id" = "li"."wbs_id")))
     JOIN "public"."contracts" "c" ON (("c"."id" = "w"."contract_id")))
     JOIN "public"."contract_organizations" "co" ON (("co"."contract_id" = "c"."id")))
     JOIN "public"."profiles" "p" ON (("p"."organization_id" = "co"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = 'Project Manager'::"public"."user_role") AND ("li"."id" = "line_item_crew_assignments"."line_item_id"))))));



CREATE POLICY "Project managers can manage line item equipment assignments" ON "public"."line_item_equipment_assignments" TO "authenticated" USING (("public"."check_is_admin"() OR (EXISTS ( SELECT 1
   FROM (((("public"."line_items" "li"
     JOIN "public"."wbs" "w" ON (("w"."id" = "li"."wbs_id")))
     JOIN "public"."contracts" "c" ON (("c"."id" = "w"."contract_id")))
     JOIN "public"."contract_organizations" "co" ON (("co"."contract_id" = "c"."id")))
     JOIN "public"."profiles" "p" ON (("p"."organization_id" = "co"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = 'Project Manager'::"public"."user_role") AND ("li"."id" = "line_item_equipment_assignments"."line_item_id")))))) WITH CHECK (("public"."check_is_admin"() OR (EXISTS ( SELECT 1
   FROM (((("public"."line_items" "li"
     JOIN "public"."wbs" "w" ON (("w"."id" = "li"."wbs_id")))
     JOIN "public"."contracts" "c" ON (("c"."id" = "w"."contract_id")))
     JOIN "public"."contract_organizations" "co" ON (("co"."contract_id" = "c"."id")))
     JOIN "public"."profiles" "p" ON (("p"."organization_id" = "co"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = 'Project Manager'::"public"."user_role") AND ("li"."id" = "line_item_equipment_assignments"."line_item_id"))))));
...
ALTER TABLE "public"."calculator_functions" ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE "public"."line_item_crew_assignments" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."line_item_entries" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."line_item_equipment_assignments" ENABLE ROW LEVEL SECURITY;
...
CREATE PUBLICATION "public_realtime" FOR ALL TABLES WITH (publish = 'insert, update, delete, truncate');


ALTER PUBLICATION "public_realtime" OWNER TO "postgres";
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."contract_organizations";
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."crew_members";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."crews";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."equipment_assignments";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."job_titles";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."line_item_crew_assignments";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."line_item_equipment_assignments";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."line_item_templates";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."line_items";
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."wbs";
...
GRANT ALL ON FUNCTION "public"."calculate_cy"("length" numeric, "width" numeric, "depth" numeric) TO "anon";
GRANT ALL ON FUNCTION "public"."calculate_cy"("length" numeric, "width" numeric, "depth" numeric) TO "authenticated";
GRANT ALL ON FUNCTION "public"."calculate_cy"("length" numeric, "width" numeric, "depth" numeric) TO "service_role";



GRANT ALL ON FUNCTION "public"."calculate_sy"("length" numeric, "width" numeric) TO "anon";
GRANT ALL ON FUNCTION "public"."calculate_sy"("length" numeric, "width" numeric) TO "authenticated";
GRANT ALL ON FUNCTION "public"."calculate_sy"("length" numeric, "width" numeric) TO "service_role";



GRANT ALL ON FUNCTION "public"."calculate_tons"("volume" numeric, "density" numeric) TO "anon";
GRANT ALL ON FUNCTION "public"."calculate_tons"("volume" numeric, "density" numeric) TO "authenticated";
GRANT ALL ON FUNCTION "public"."calculate_tons"("volume" numeric, "density" numeric) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."create_clone_for_test_user"("input_session_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."create_clone_for_test_user"("input_session_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_clone_for_test_user"("input_session_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."create_clone_for_test_user"("session_id" "uuid", "user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."create_clone_for_test_user"("session_id" "uuid", "user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_clone_for_test_user"("session_id" "uuid", "user_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."create_clone_for_test_user"("session_id" "uuid", "user_id" "uuid", "base_contract_id" "uuid", "base_organization_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."create_clone_for_test_user"("session_id" "uuid", "user_id" "uuid", "base_contract_id" "uuid", "base_organization_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_clone_for_test_user"("session_id" "uuid", "user_id" "uuid", "base_contract_id" "uuid", "base_organization_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."ensure_test_user_organization"() TO "anon";
GRANT ALL ON FUNCTION "public"."ensure_test_user_organization"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."ensure_test_user_organization"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."lock_budget_on_status_change"() TO "anon";
GRANT ALL ON FUNCTION "public"."lock_budget_on_status_change"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."lock_budget_on_status_change"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."prevent_daily_log_if_inactive"() TO "anon";
GRANT ALL ON FUNCTION "public"."prevent_daily_log_if_inactive"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."prevent_daily_log_if_inactive"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_contract_budget"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_contract_budget"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_contract_budget"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."validate_formula_unit"() TO "anon";
GRANT ALL ON FUNCTION "public"."validate_formula_unit"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."validate_formula_unit"() TO "service_role";



GRANT ALL ON FUNCTION "public"."validate_formula_units"() TO "anon";
GRANT ALL ON FUNCTION "public"."validate_formula_units"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."validate_formula_units"() TO "service_role";
...
GRANT ALL ON TABLE "public"."asphalt_types" TO "anon";
GRANT ALL ON TABLE "public"."asphalt_types" TO "authenticated";
GRANT ALL ON TABLE "public"."asphalt_types" TO "service_role";
...
GRANT ALL ON TABLE "public"."calculator_functions" TO "anon";
GRANT ALL ON TABLE "public"."calculator_functions" TO "authenticated";
GRANT ALL ON TABLE "public"."calculator_functions" TO "service_role";
...
GRANT ALL ON TABLE "public"."contract_organizations" TO "anon";
GRANT ALL ON TABLE "public"."contract_organizations" TO "authenticated";
GRANT ALL ON TABLE "public"."contract_organizations" TO "service_role";
...
GRANT ALL ON TABLE "public"."dump_trucks" TO "anon";
GRANT ALL ON TABLE "public"."dump_trucks" TO "authenticated";
GRANT ALL ON TABLE "public"."dump_trucks" TO "service_role";
...
GRANT ALL ON TABLE "public"."line_item_crew_assignments" TO "anon";
GRANT ALL ON TABLE "public"."line_item_crew_assignments" TO "authenticated";
GRANT ALL ON TABLE "public"."line_item_crew_assignments" TO "service_role";



GRANT ALL ON TABLE "public"."line_item_entries" TO "anon";
GRANT ALL ON TABLE "public"."line_item_entries" TO "authenticated";
GRANT ALL ON TABLE "public"."line_item_entries" TO "service_role";



GRANT ALL ON TABLE "public"."line_item_equipment_assignments" TO "anon";
GRANT ALL ON TABLE "public"."line_item_equipment_assignments" TO "authenticated";
GRANT ALL ON TABLE "public"."line_item_equipment_assignments" TO "service_role";



GRANT ALL ON TABLE "public"."line_item_templates" TO "anon";
GRANT ALL ON TABLE "public"."line_item_templates" TO "authenticated";
GRANT ALL ON TABLE "public"."line_item_templates" TO "service_role";
...
GRANT ALL ON TABLE "public"."tack_rates" TO "anon";
GRANT ALL ON TABLE "public"."tack_rates" TO "authenticated";
GRANT ALL ON TABLE "public"."tack_rates" TO "service_role";
...
GRANT ALL ON TABLE "public"."wbs" TO "anon";
GRANT ALL ON TABLE "public"."wbs" TO "authenticated";
GRANT ALL ON TABLE "public"."wbs" TO "service_role";
