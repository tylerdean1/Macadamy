      ...
      <Paper elevation={1} sx={{ p: 3, mb: 3, bgcolor: 'background.default' }}>
        ...
        <Stack spacing={2}>
          <Box>
            ...
            <Typography variant="body1">{contractData.title || 'Not provided'}</Typography>
          </Box>

          <Box>
            <Typography variant="subtitle2">Location</Typography>
            <Typography variant="body1">{contractData.location || 'Not provided'}</Typography>
          </Box>

          <Box>
            <Typography variant="subtitle2">Status</Typography>
            <Typography variant="body1">{contractData.status || 'Not provided'}</Typography>
          </Box>
          ...
          <Box>
            ...
            <Typography variant="body1">{formatDate(contractData.start_date)}</Typography>
          </Box>

          <Box>
            <Typography variant="subtitle2">End Date</Typography>
            <Typography variant="body1">{formatDate(contractData.end_date)}</Typography>
          </Box>
      ...
      <Paper elevation={1} sx={{ p: 3, mb: 3, bgcolor: 'background.default' }}>
        ...
        <List dense>
          {wbsSections.map((section, index) => (
            <ListItem key={`wbs-${index}`} divider={index < wbsSections.length - 1}>
              <ListItemText
        ...
        <Paper elevation={1} sx={{ p: 3, mb: 3, bgcolor: 'background.default' }}>
          ...
          <List dense>
            {mapLocations.map((map, index) => (
              <ListItem key={`map-${index}`} divider={index < mapLocations.length - 1}>
                <ListItemText
        ...
        <Paper elevation={1} sx={{ p: 3, mb: 3, bgcolor: 'background.default' }}>
          ...
          <List dense>
            {lineItems.map((item, index) => (
              <ListItem key={`item-${index}`} divider={index < lineItems.length - 1}>
                <ListItemText
